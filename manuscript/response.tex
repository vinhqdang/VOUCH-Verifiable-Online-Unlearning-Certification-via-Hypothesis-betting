...
states the convention at the point where it could otherwise be misread. Equation~(2) is now
...
in a single run on TOFU/GPT-2, over the same three seeds as the main benchmark tier, so
every row shares one fine-tuned adapter, one cohort and one machine per seed. RMU
certifies at $\eps = 0.2$ on all three seeds, with a realised cohort advantage of
$+0.050$ --- close to what retraining a fresh adapter achieves on the same cohorts
($+0.026$), against the un-unlearned model's $+0.097$ --- at a retain NLL of $2.55$
against retraining's $1.99$.
...
forget split's likelihood at all ($2.73$ against the un-unlearned model's $2.19$),
whereas NPO and SimNPO raise it to $6.34$ and $5.79$ --- yet all three land at a
comparable realised advantage ($+0.050$, $-0.014$, $-0.035$). A
forget-quality metric reading likelihoods would rank RMU far below the other two; the
certificate treats them as comparable, because what it measures is whether an attacker
...
two readings coming apart on real models. The un-unlearned control itself disagreed
across seeds --- revoked twice, issued once at a realised advantage of $+0.042$ --- the
same phenomenon \S\ref{sec:exp-bench} already reports for two cells of the main
benchmark: $\eps = 0.2$ is loose enough that a weakly-memorising run can certify
without any unlearning having occurred.

The tier is also no longer single-architecture or single-benchmark. The identical
protocol repeated unchanged on TOFU/Pythia-160M (new \S\ref{sec:exp-rmu-pythia}, Table~\ref{tab:rmu-pythia})
reproduces the same pattern --- RMU certifies at $\eps=0.2$ and $0.1$ on all three
seeds and turns undetermined at $0.05$, at a realised advantage close to retraining's
and a forget-NLL shift far smaller than NPO's or SimNPO's --- so the loss/detectability
divergence is not a property of one model family. On MUSE-News/GPT-2 (new
\S\ref{sec:exp-rmu-muse}, Table~\ref{tab:rmu-muse}) most of that pattern transfers too, but not all of
it: RMU is revoked at $\eps=0.2$ on one of three seeds there, which it never is on
TOFU at that tolerance, and we report the difference rather than smooth it over.
...
\citep{jagielski2023measuring}. The comparison it draws is the useful one:
exposure answers ``how memorised is this string,'' whereas the ghost
...
We have accordingly softened the claim exactly as you ask. The phrase ``the largest
residual advantage any score in the class can extract'' is gone; the guarantee is
written as a statement about $\F$ in the abstract, the introduction's scope paragraph,
the scores subsection, the scope paragraph of the theory section and the conclusion.
We are also careful not to over-read the table in the other direction: a null result
does not establish that the class restriction is immaterial, which is why
\S\ref{sec:exp-lira} goes on to run an attack strong enough to say something.
...
about how much it carries. It is $16$ shadows against the hundreds a canonical LiRA
would use, so it does not show that $\F$ is wide enough in general. But unlike the
stratum-restricted and learned-combination attacks, it cannot be dismissed as too weak
to have found anything: it finds a great deal where leakage exists and nothing where
the certificate says there is none. That is the first measurement in the paper that
\emph{bounds} the class-restriction gap rather than merely naming it.

The result is also no longer confined to the TinyGPT stand-in: the identical attack,
unchanged in design, run at full scale ($24$ shadows, $384$ pairs) on TOFU/GPT-2 ---
the architecture our own certified NPO row uses --- reproduces the same pattern (new
\S\ref{sec:exp-lira-gpt2}, Table~\ref{tab:lira-gpt2}): $\hat\Delta = +0.479$
(95\% CI $[+0.385,+0.566]$) on the un-unlearned model, $\hat\Delta = -0.031$
(95\% CI $[-0.133,+0.071]$) on the certified one, again excluding every tolerance we
test. So this is now one unlearning method across two architectures and shadow
counts of $16$ and $24$, not a single stand-in tier.
\end{reply}
...
\begin{reply}
Agreed, and it was as cheap as you expected. \S\ref{sec:exp-descriptive} and
...
(``which is our own earlier design, not a published method''), the experiments
section, and the caption of the soundness table.
\end{reply}
...
\noindent Since the first revision draft we have closed five of the six gaps this
section previously listed: the LiRA-style attack is run (\S\ref{sec:exp-lira}), RMU is added as a
second utility-preserving subject (\S\ref{sec:exp-rmu}) and its tier brought to the same
three seeds as the main benchmark tier, the paraphrase-aware score is implemented
...
\item \textbf{The LiRA result's reach.} It now runs on two architectures ---
TinyGPT ($16$ shadows) and, at full scale, the TOFU/GPT-2 tier our own certified NPO
row uses ($24$ shadows, \S\ref{sec:exp-lira-gpt2}) --- against one unlearning method
in each case, with the same pattern on both: a large, tolerance-exceeding advantage on
the un-unlearned model and a null bounded away from every tolerance on the certified
one. A shadow study at 1B+ parameters still needs the whole pipeline repeated per
shadow, which we could not afford, and nothing here rules out a larger model or a
different unlearning method behaving differently.
...
\item \textbf{The RMU tier's coverage, and one place it did not transfer.} Three seeds
throughout, matching the main benchmark tier but reported separately rather than
merged into it. It now spans two architectures on TOFU (GPT-2 and Pythia-160M,
\S\ref{sec:exp-rmu}--\S\ref{sec:exp-rmu-pythia}, same pattern on both) and a second benchmark
(MUSE-News/GPT-2, \S\ref{sec:exp-rmu-muse}), where most of the pattern again transfers but one
part does not: RMU is revoked at $\eps=0.2$ on one of three MUSE seeds, something it
never does on TOFU at that tolerance. We read that as a real, modest difference in
RMU's margin rather than noise, and report it rather than average it away. Larger
models remain untested for this method.
\end{enumerate}

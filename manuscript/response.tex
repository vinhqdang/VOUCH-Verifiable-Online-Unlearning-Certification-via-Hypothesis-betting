...
Theorem~2, restated; Theorem~3 (power) is now Theorem~4; Theorem~4 (streaming) is now
Theorem~5; Proposition~6 (bridge) is now Proposition~7; Table~3 (power against the
...
(\S5.1, Tables~1--2): under model-level over-dispersion the previous fixed-boundary
...
\item \textbf{Four valid sequential comparators added} (\S5.4, Table~5): exact
...
size.
\item \textbf{Tolerance sweep to $\eps = 0.05$ on real models} (Table~11), with the
...
min-$k\%$ on identical runs (\S5.10); attacks from \emph{outside} the declared class
(\S5.11); canary detectability, with the negative result and the entropy-dilution
design rule that follows from it (\S5.12).
...
\emph{(a) Which randomness defines the null (\S3.3, \S4.1).} The randomness is the
...
\emph{(d) Simulations with dependent and heterogeneous signs (\S5.1, Tables~1--2).}
...
We note in \S4.2 one honest consequence of the retargeting: a cohort audited to
...
you suggest, in the strongest form our data supports (\S5.10, Table~15): on identical
...
\emph{The threat model.} The honest-but-verifiable assumption is now stated in \S3.1,
...
cannot drift again. \S3.4 now lists every member of $\F$ with the orientation stated
...
\begin{reply}
You are exactly right, including in your inference about Table~3 (now Table~6). Theorem~4 now carries
...
\begin{reply}
Added as Remark~3, distinguishing three statements. Each score's sequence covers
...
all four comparators you name (\S5.4, Table~5), on the same sign stream and the same
query budget:
...
Table~12 gives every pooled verdict count with a Clopper--Pearson $95\%$ interval:
$16/18 \to [0.65, 0.99]$, $18/18 \to [0.81, 1.00]$, $17/18 \to [0.73, 1.00]$. Mean
columns carry across-seed standard deviations. Every claim in \S5.8 is now stated as
...
(Table~13) and named in \S5.8 as the best-behaved of the approximate methods on the
utility axis.
...
(\S5.1, \S5.4, \S5.10, \S5.11, \S5.12) that speak to the framework's validity rather
...
Results are in the new Table~13, on TOFU/GPT-2 over $3$ seeds.
...
unavailable in Table~13 with the reason given.
\end{reply}
...
\begin{reply}
All four are now discussed substantively, in a restructured \S2.3 titled
...
correction), point to our new out-of-class experiments (\S5.11) as a first step, and
...
We additionally added, on your and Reviewer~2's prompting, a new \S2.2
...
\emph{End-to-end at $\eps \le 0.05$ (Table~11).} The corrected finite-cohort target
...
$\eps = 0.2$ --- Theorem~4 says it should not, since $\eps = 0.1$ needs about $600$
...
Both targets are reported side by side in Tables~11 and 14 and we recommend reading them
together.
...
(\S5.9) and named it in the conclusion as the most important open problem the
framework faces.

\emph{What $\eps = 0.2$ means, said plainly.} \S5.8 now closes with an explicit
...
point about the permissiveness of the default, and we now use it as such in \S5.8.
\end{reply}
...
extrapolation is weakest exactly where it matters most. \S5.10's head-to-head against
...
\begin{reply}
Done, as Remark~4 placed immediately after the proposition, and stated at least as
...
either direction. The phrase ``semantic anchor'' is deleted; \S2.4 now calls it a
...
(\S5.11, Table~16). Two attacks that no member of $\F$ implements were run against all
...
revision. We say this in \S5.11 and name it as the most valuable next experiment,
...
\begin{reply}
You were right that it was cheap and right that we should have done it. \S5.10 and
Table~15 now report both metrics on the same models and the same seeds, and the specific
...
\emph{Intervals and dispersion.} New Table~12 gives every pooled verdict count with a
...
number. The caption states the censoring convention, and \S5.5 explains the artefact
...
(\S5.12, Table~17). A trained character-$n$-gram classifier does the same.
...
asked for, and we report both numbers in \S3.4 and \S5.8 rather than only the reassuring
...
\begin{reply}
Added, in \S5.2 immediately after the calibration numbers: ``A realised error of
...
\begin{reply}
The soundness result is now carried by the table and Remark~2 alone, and the redundant
...
identified two further gaps ourselves; we would rather list all five than let them be
inferred from the text.

\begin{enumerate}[leftmargin=1.4em,itemsep=3pt]
\item \textbf{A LiRA-style attack outside $\F$} (R2.W4). Not run; it needs dozens of
retrained shadow references per cell. The two attacks we could run came back below
their own binomial nulls, so the size of the gap between $\sup_{s\in\F}|\Delta^{(s)}|$
and the advantage a genuinely strong attack could extract is, on our evidence,
\emph{unmeasured} rather than small. This is the single most consequential thing this
revision does not settle.
...
\item \textbf{A finite-population power analysis} for the cohort target. Theorem~4 is
proved for the fixed-boundary process with i.i.d.\ signs; under without-replacement
reveal the increments are neither independent nor identically distributed and the
regret bounds do not transfer as stated. We have scoped the theorem accordingly and
report the cohort target's certification times empirically, but the matching theory is
missing.
\item \textbf{An audit-side source of reveal randomness.} Theorem~2 needs the reveal
order to be independent of the realised signs. Our runs seed it from the run seed,
which also seeds cohort generation and training. Nothing in the pipeline exploits the
coupling, but the condition is not met and we say so in Remark~1 and Appendix~A rather
than assert independence we do not have.
\end{enumerate}
...
head-to-head against the metrics we claim to replace, and a detectability measurement
that overturns a claim we should never have made without testing. The out-of-class
experiment we would characterise differently: it is the one addition that came back
inconclusive, and we report it as such rather than as support for a conclusion it does
not carry. We are grateful for both reports.

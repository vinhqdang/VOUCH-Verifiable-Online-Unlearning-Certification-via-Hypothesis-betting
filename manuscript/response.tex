...
frozen reference replaced by a constant margin --- and ran it end to end under identical
seeds, canary manifests and budgets as the existing subjects, so it is directly
comparable. It certifies like NPO while holding held-out loss closest to the retrained
reference of any approximate method we test, and is now reported in the benchmark table
and named in \S6.9 as the best-behaved subject on the utility axis.
...
\begin{reply}
Agreed. The end-to-end runner now records, behind an \texttt{--extra-metrics} flag and
enabled for the revision-round runs: mean NLL on the benchmark's own \emph{forget} and
\emph{retain} splits; greedy-decode ROUGE-L recall against the reference continuation
on both splits, which is the TOFU/MUSE reporting convention; and a
\texttt{capability\_nll} on a fixed probe set disjoint from both splits --- TOFU
\texttt{world\_facts} for TOFU and the MUSE holdout news stream for MUSE. The
implementation and definitions are in Appendix~A. We are explicit in the appendix that
\texttt{world\_facts} is a modest general-knowledge probe rather than a full capability
suite such as MMLU, and that a broader suite is the natural next step; we would rather
report a small, clearly described probe than imply coverage we do not have.
\end{reply}

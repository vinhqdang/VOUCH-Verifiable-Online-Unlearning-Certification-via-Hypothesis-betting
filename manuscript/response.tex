...
The tier is also no longer single-architecture: the identical protocol repeated
unchanged on TOFU/Pythia-160M (new \S\ref{sec:exp-rmu-pythia}, Table~\ref{tab:rmu-pythia}) reproduces the
same pattern --- RMU certifies at $\eps=0.2$ and $0.1$ on all three seeds and turns
undetermined at $0.05$, at a realised advantage close to retraining's and a forget-NLL
shift far smaller than NPO's or SimNPO's --- so the loss/detectability divergence is
not a property of one model family. Task-vector negation remains unevaluated, MUSE is
untested for this method, and we still name the method list as incomplete.
\end{reply}
...
\item \textbf{The RMU tier is TOFU only.} Three seeds, matching the main benchmark
tier, but reported separately rather than merged into it, and now checked on two
architectures (GPT-2 and Pythia-160M, \S\ref{sec:exp-rmu}--\S\ref{sec:exp-rmu-pythia}) with the same
pattern on both; MUSE and larger models remain untested for this method.
\end{enumerate}

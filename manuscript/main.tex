...
\abstract{Unlearning methods for large language models honour deletion requests with no
...
(Section~\ref{sec:exp-outclass}) do \emph{not} beat the noise floor of subsampling and
...
falsely certified a leaking model $99.6\%$ of the time under an adversarially
constructed composite null; the sound alternative requires every score's own process
to concur. \emph{(iii)} Certificates and alarms must compose
...
directions, power that matches the Kullback--Leibler information limit, an exact
finite-cohort certification-time bound that holds for every betting strategy and every
reveal order, and a one-directional consistency bridge from certified
$(\eps_u,\delta_u)$-unlearning (Section~\ref{sec:theory}). We report a comprehensive empirical study
...
end-to-end certification of gradient ascent, GradDiff, NPO and retraining on TOFU and
MUSE-News across three architectures, with SimNPO and representation misdirection
added on TOFU/GPT-2 and the latter confirmed on a second architecture and a second
benchmark, a tolerance sweep down to $\eps = 0.05$, utility guardrails, and
dose--response calibration; a head-to-head
...
erasure is behaviourally uncertifiable, so we do not claim it. The class restriction
...
\citet{li2026beliefs} motivate is implemented and added to $\F$
...
certificates; the super-population arm reaches $\eps = 0.1$ once the cohort clears
...
regime the canary-to-organic extrapolation needs it. The guarantee is
...
VOUCH does not itself supply. The audit's own randomness is now as checkable as its
...
seed-derived order. Canaries must be planted before fine-tuning, so legacy

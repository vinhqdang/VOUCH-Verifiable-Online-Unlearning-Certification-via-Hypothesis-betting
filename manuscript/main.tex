...
\emph{not} beat the noise floor of subsampling, so they neither show $\F$ is too
...
MUSE-News across three architectures, with SimNPO added on TOFU/GPT-2, a tolerance
sweep down to $\eps = 0.05$, utility guardrails, and dose--response calibration; a head-to-head
...
erasure is behaviourally uncertifiable, so we do not claim it. The class restriction is a real limit on the claim, though our attempt to measure it
